...
\end{lemma}
\begin{proof}
	Let $\gamma_{g, \phi}^{k+1}=\gamma_{g, \phi}(x^{k+1})$ be the $\phi_k$-subgradient of $g(\cdot)$ at $x^{k+1}$. By definition of $\phi$-subdifferential, $\gamma_{g, \phi}^{k+1}$ satisfies both the following inequality,
	\begin{align}
    ...
    \end{align}
	and the approximate first-order optimality conditions 
    \begin{align}
    ...
    \begin{align*}
		...
		\frac{1}{M_k}\|  \nabla f(x^k) + \gamma_{g, \phi}^{k+1} \|- \frac{\sqrt{2M_k\phi_k}}{M_k}.
	\end{align*}
	With \eqref{equ:_g_phi_subgradient} and \eqref{equ:_bound_Q_Qstar_1st_optima}, we derive \eqref{equ:bound_Qk_Qk1_inexact} as follows,
	\begin{align*}
	        &Q(H_k,x^k,x^k) - Q(H_k,x^{k+1}, x^k) \\
	        % \nonumber
	        &= g(x^k) - g(x^{k+1}) 
...
\begin{proof}	                
	...
	From convexity and our assumptions \eqref{eq:bnddelta} holds, as in the exact case.
	 ...
	 \[
	  \Delta_k\leq D \|\nabla f(x^k)+\gamma_{g, \phi}^{k+1}\| +  \frac{2(1+\theta)L_g\sqrt{2M_k\phi_k}}{\sigma_k}\leq (2D+  \frac{2(1+\theta)L_g}{\sigma_k})\sqrt{2M_k\phi_k}.
	  \]
	...
	%  Using Lemma 5 in the old version of the paper we have   
	%   \begin{align}
	%        &Q(H_k,x^k,x^k) - Q(H_k,x^{k+1}, x^k)\geq \frac{\sigma_k}{8}(\|x^{k+1}-x^k\|^2-\phi).  \nonumber 
	%        \end{align}
	%        
	%        Substituting bound on $\|x^{k+1}-x^k\|^2$ from \eqref{eq:bndcase2} and on $\phi$ from \eqref{eq:case2} we have 
	%\begin{eqnarray}
	%        Q(H_k,x^k,x^k) - Q(H_k,x^{k+1}, x^k)&\geq& \frac{\sigma_k}{16M_k}  \| \nabla f(x^k) + \gamma_g^{k+1} \|^2 
	%        - \frac{\sigma_k}{64M_k}  \| \nabla f(x^k) + \gamma_g^{k+1} \|^2 \nonumber \\
	%        &\geq& \frac{3\sigma_k}{64M_k}  \| \nabla f(x^k) + \gamma_g^{k+1} \|^2 
	...
	%        \end{eqnarray}
	 ...
	 Using the inexact optimality condition for $Q$ at $x^{k+1}$ and convexity of $g$ we have
	        ...
	        &Q(H_k,x^k,x^k) - Q(H_k,x^{k+1}, x^k)
	        = g(x^k) - g(x^{k+1}) -  \nabla f(x^k)^T( x^{k+1}-x^k)  - \frac{1}{2}\|x^{k+1}-x^k\|^2_{H_k} \nonumber \\
	        \nonumber&\geq ( \gamma_{g, \phi}^{k+1})^T( x^{k}-x^{k+1} ) + \|x^{k+1}-x^k\|_{H_K}^2 +  (\gamma_{g, \phi}^{k+1})^T( x^{k+1}-x^k )+ \eta_k^T( x^{k+1}-x^k ) - \frac{1}{2}\|x^{k+1}-x^k\|^2_{H_k} \\
	          \label{eq:lowerbndinexact}
