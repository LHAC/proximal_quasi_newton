...
\begin{assumptions}$\\$ % let list start right below
...
\end{theorem}
\proof{A proof follows trivially from Theorem  \ref{th:inexact_conv_rate}. }
...
%\linesnumberedhidden \dontprintsemicolon 
Select $0<\beta<1$ and set $\mu=\bar \mu$\; \For{$i=1,2,\cdots$}{
Define $H=G+\frac{1}{2\mu} I$,  and compute $p(x):=p_{H,\phi}(x)$\\ by applying {\em RCD \ }$(Q(H,  v,x), x, l(k))$\;
If $F(p(x))- F(x) \leq \rho (Q(H,  p(x),x)- F(x))$, then output $H$ and $p(x)$, {\bf Exit} \;
Else $\mu=\beta^{i}\bar  \mu$\;
}
\end{algorithm2e}
...
\begin{figure}[!t]
    ...
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-Leukemia}
       \label{fig:Leukemia}
    }
    ~
    \subfigure[ $S: 1869 \times 1869$ ]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-hereditarybc}
       \label{fig:hereditarybc}
    }
    ~
    \subfigure[ $S:  692 \times 692$ ]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-ER_692}
       \label{fig:Leukemia}
    }
    ~
    \subfigure[ $S: 834 \times 834$ ]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-Arabidopsis}
       \label{fig:hereditarybc}
    }
...
\begin{figure}[!t]
    \centering
    \subfigure[The number of coordinate descent steps on subproblems w.r.t. outer iteration.]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-SICS-cd-iter}
    ...
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-SICS-obj-cd}
       \label{fig:sics-cd-obj}
    }
    ~ 
    \subfigure[The number of coordinate descent steps on subproblems w.r.t. outer iteration.]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-slr-cd-iter}
       \label{fig:slr-cd-iter}
    }
    ~
    \subfigure[ Both axes are in log scale. Change of objective w.r.t. the number of coordinate descent steps.]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-slr-obj-cd}
       \label{fig:slr-cd-obj}
    } 
    % \caption{ RCD step count of  LHAC on different SLR data sets. }
    % \label{fig:slr-cd}   
    \caption{ RCD step count of LHAC on different SLR and SICS data sets. }
    \label{fig:sics-cd}   
\end{figure}
...
\begin{figure}[!t]
    \centering
    \subfigure[ a9a ($123, 32561$)]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-a9a}
       \label{fig:a9a}
    }
    ~
    \subfigure[ slices ($385, 53500$)]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-slice}
       \label{fig:slices}
    }
    ~    
    \subfigure[ gisette ($5000, 6000$)]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-gisette_scale}
       \label{fig:gisette}
    }
    ~
    \subfigure[epsilon ($2000, 1e^5$)]
    {
       \includegraphics[width=0.2\columnwidth]{resources/fig-epsilon_normalized}
       \label{fig:connect}
    }
