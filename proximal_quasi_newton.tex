...
Let $x^{k+1}:=\arg\min_{u\in\br^n} Q(H_k, u,x_k)$. Then the decrease on the function $Q$ can be bounded from below by a quadratic function in terms of the step size, i.e., $\|x^{k+1} - x^k\|$,
\begin{align}
	\label{equ:bound_Qk_Qk1}
    Q(H_k,x^k,x^k) - Q(H_k,x^{k+1}, x^k) \geq \frac{\sigma_k}{2}\|x^{k+1}-x^k\|^2.
\end{align}
And There exists such vector $\gamma_g^{k+1} \in \partial g(x^{k+1})$ that the following bounds on the step size hold: 
\begin{align}
...
\end{lemma}
\begin{proof}
     The proof is a special case of Lemma~\ref{lem:bound_step_size_inexact} when $\phi_k$ is equal to zero.
\end{proof}
...
\begin{proof}
	...
	\end{equation}
	Indeed $F(x^{k})-F(x^{k+1})\geq \rho (Q(H_k, x^{k}, x^k)-Q(H_k,x^{k+1}k, x^k))$. And a bound on the reduction in $Q$ can be established by combining \eqref{equ:bound_Qk_Qk1} and \eqref{equ:bound_step_size},
    \begin{align*}
...
Suppose that Assumptions~\ref{as:exact_conv_rate} hold. 
Assume that all iterates $\{x^k\}$ of inexact Algorithm \ref{alg:ISTA-SD} are generated with some $\phi_k>0$ that satisfy
\begin{align}
\label{equ:phi_conv_rate_required}
    \phi_k \leq \frac{\alpha_s}{k^2}, \mbox{ with } \alpha_s > 0.
\end{align}
Let $k_0$ be the smallest integer such that $\phi_{k_0} \leq \frac{\alpha_s}{k_0^2} \leq 1$.
Let constants $a = \min\{c, \frac{1}{b\sqrt{\alpha_s}}\}$ and $b, c$ be given as follows,
\begin{align}
\label{equ:bound_b_c}
    b = \theta D_{\Xcal_0}\sqrt{2M}+  \frac{2(1+\theta)L_g}{\sigma}\sqrt{2M} + 1, 
    ~
    c = \frac{\rho \left( \frac{\sigma^2}{M^2}(1 - \psi)^2 - (2\psi + 3\psi^2) \right)}{2(D_{\Xcal_0} + \frac{2L_g(1+\psi)}{\sqrt{\sigma}} + \frac{\psi}{\sqrt{2}})^2}.
\end{align}
where $\psi = \frac{1}{\theta}$ and $\theta$ is the same as specified in Lemma~\ref{lem:bound_iter_inexact}. Then for any $k \geq k_0$
\begin{align}
    \label{eq:bound_F_F*}
	F(x^k) - F(x^*) 
    \leq \max\{b \frac{\sqrt{\alpha_s}}{k}, \frac{1}{a(k-1)}, \frac{1}{c(k-k_0+1)} \}.
\end{align}
\end{theorem}
\begin{proof}
	For all $k \geq k_0$, let $b$ and $c$ denote the uniform upper and lower bounds, respectively, for $b_k$ and $c_k$ in \eqref{equ:bound_bk_ck}, i.e., $b_k \leq b$, $c_k \geq c, \forall k\geq k_0$. Then from Assumptions~\ref{assub:bound_h} we can derive the expressions for $c$ as follows,
	\begin{align*}
	    c_k
	...
	\end{align*}
	from which \eqref{equ:bound_b_c} follows by substituting $\psi = \frac{1}{\theta}$. And $b$ can be obtained in a similar fashion.

	Now consider any iteration $k \geq k_0$. From Lemma~\ref{lem:bound_iter_inexact}, it follows that either \eqref{equ:bound_F_inexact_case1} or \eqref{equ:bound_F_inexact_case2} must hold. First we suppose \eqref{equ:bound_F_inexact_case1} hold and obtain
	\begin{align}
	\label{equ:_thm_conv_rate_case1}
	    F(x^k) - F(x^*) = \Delta F_k \leq b_k \sqrt{\phi_k} \leq b \frac{\sqrt{\alpha_s}}{k}
	\end{align}
	Next, we consider the other case where \eqref{equ:bound_F_inexact_case2} holds. Let $k_1 \geq k_0$ denote the largest index of such iteration before $k$, i.e., $k_1 < k$, that \eqref{equ:bound_F_inexact_case1} holds. If no such $k_1$ exists, then \eqref{equ:bound_F_inexact_case2} hold for all $k \geq k_0$. Summing \eqref{equ:bound_F_inexact_case2} from $k_0$ till $k$, we obtain,
	\begin{align*}
	    \frac{1}{\Delta F_k}  \geq c(k - k_0 + 1)  + \frac{1}{\Delta F_{k_0}} \geq c(k - k_0 + 1)
	\end{align*}
	which implies,
	\begin{align}
	\label{equ:_thm_conv_rate_case2}
	    F(x^k) - F(x^*) \leq \frac{1}{c(k - k_0 + 1)}
	\end{align}
	On the other hand, if such $k_1$ exists, then
	\begin{align}
	\label{equ:_thm_conv_rate_Fk1_upper}
	    \Delta F_{k_1} \leq b \frac{\sqrt{\alpha_s}}{k_1}
	\end{align}
	and \eqref{equ:bound_F_inexact_case2} holds true for all iteration from $k_1+1$ to $k$, i.e.,
	\begin{align*}
	    \frac{1}{\Delta F_{k_1 + i + 1}} - \frac{1}{\Delta F_{k_1 + i}} \geq c, ~\forall i = 1, ..., k-k_1
	\end{align*}
	Summing up the above inequalities for $i$ from $1$ to $k-k_1$ and using \eqref{equ:_thm_conv_rate_Fk1_upper} we obtain the following bound on $\Delta F_{k+1}$, with $a = \min\{c, \frac{1}{b\sqrt{\alpha_s}}\}$,
	\begin{align*}
	    \frac{1}{\Delta F_{k + 1}} \geq c (k-k_1) + \frac{1}{\Delta F_{k_1 + 1}} \geq c (k-k_1) + \frac{1}{\Delta F_{k_1}} \geq c (k-k_1) + \frac{k_1}{b\sqrt{\alpha_s}} \geq a k.
	\end{align*}
	which implies,
	\begin{align}
