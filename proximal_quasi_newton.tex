...
\usepackage{amsxtra, amsfonts,amscd,  amssymb, graphicx,subfigure}
...
\end{align*}
and  Lemma \ref{lem:subp_inexact} holds at each iteration $k$ of Algorithm \ref{alg:ISTA-SD} with  $\epsilon=-\frac{1-\rho}{\rho}(F(x^{k+1})- F(x^k))$. 
...
We now derive the complexity bound for  the exact version of Algorithm \ref{alg:ISTA-SD}. 

\begin{theorem}\label{the:ISTA-SD}
In Algorithm \ref{alg:ISTA-SD}, suppose that at iteration $k$,  $H_k$, is chosen so that the sufficient decrease condition \eqref{eq:dec_cond} holds.
Then the iterates $\{x^k\}$ in Algorithm \ref{alg:ISTA-SD} satisfy
\begin{align}\label{the:ISTA-nonsmooth-conclude} 
F(x^k)-F(x^*)\leq \frac{\|x^0-x^*\|_{H_0}^2}{2k}+\frac{(1-\rho)(F(x^0)-F(x^*))}{2k\rho}+\frac{\sum_{i=0}^{k-1}M_{i+1}\|x^i-x^{i+1}\|^2}{2k}, \quad\forall k, \end{align}
where $x^*$ is an optimal solution of \eqref{prob:P} and $M_{i+1}=\|H_{i+1}-H_i\|$. Thus, the sequence $\{F(x^k)\}$ produced by Algorithm \ref{alg:ISTA-SD} converges to $F(x^*)$ if  
\[
\frac{\sum_{i=0}^{k-1}M_{i+1}\| x^i-x^{i+1}\|^2}{k}\to 0\ {\rm as\ } k\to 0.
\]
Moreover,  the number of iterations needed to obtain an $\cal{\epsilon}$-optimal solution is at most $O(\frac{1}{\cal{\epsilon}})$, if 
\begin{equation}\label{eq:sumboundK}
\sum_{i=0}^{k-1}M_{i+1}\| x^i-x^{i+1}\|^2\leq K, \ {\rm  for\  some\ } K>0\   \forall k. 
\end{equation}
\end{theorem}
...
As in the exact case, we need to  establish  bound  \eqref{eq:sumboundK} to obtain global sublinear rate of convergence. We first present a lemma below, which establishes an upper bound on the step size, similar to \eqref{equ:bound_delta_x}, but in the case where the step is not required to be the exact minimizer of the quadratic function $Q(\cdot)$. In fact, the bound holds when the step can be arbitrarily far from the optimal point with the error characterized by $\phi$ defined in \eqref{lem:BT2.3-assump-g}.


\begin{lemma}
Consider some vector $v\in \br^n$,  a scalar $\phi>0$ and a matrix $H\succeq \sigma I$. Let $p_\phi(v)\equiv p_{H, \phi}(v)$ be defined as in \eqref{eq:pvphi}. Then
    \begin{align}
    \label{equ:bound_delta_x_inexact}
        || v - p_\phi(v) ||^2 \leq  \frac{8}{\sigma} ( Q(H, v, v) - Q(H, p_\phi(v), v) + \phi ).
    \end{align}
    
\end{lemma}

\begin{proof}
    We use \eqref{equ:bound_delta_x} with $H=H_k$ and  the definition of $\sigma$ to  obtain the upper bounds on the distances between $v$ and $p(v)$ (the exact minimizer of $Q(H,u,v)$) and between $p_\phi(v)$ and $p(v)$, respectively, by
    \begin{align*}
        ||v - p(v)||^2 &\leq \frac{2}{\sigma} ( Q(H, v, v) - Q(H, p(v), v) ), \\
        ||p_\phi(v) - p(v)||^2 &\leq \frac{2}{\sigma} ( Q(H, p_\phi(v), v) - Q(H, p(v), v) ).
    \end{align*}
    Taking square root on both sides of the above inequalities, we have
    \begin{align*}
        ||v - p(v)|| &\leq \sqrt{ \frac{2}{\sigma} ( Q(H, v, v) - Q(H, p(v), v) ) },\\
        ||p_\phi(v) - p(v)|| &\leq \sqrt{ \frac{2}{\sigma} ( Q(H, p_\phi(v), v) - Q(H, p(v), v) ) }.
    \end{align*}
    Then,
    \begin{align*}
        || v - p_\phi(v) || 
        &\leq ||v - p(v)|| + ||p_\phi(v) - p(v)|| \\
        &\leq \sqrt{ \frac{2}{\sigma} ( Q(H, v, v) - Q(H, p(v), v) ) } + \sqrt{ \frac{2}{\sigma} ( Q(H, p_\phi(v), v) - Q(H, p(v), v) ) }   \\
        &\leq 2\sqrt{ \frac{2}{\sigma} ( Q(H, v, v) - Q(H, p(v), v) ) }   \\
        &= 2\sqrt{ \frac{2}{\sigma} ( Q(H, v, v) - Q(H, p_\phi(v), v) + Q(H, p_\phi(v), v) - Q(H, p(v), v) ) }   \\
        &\leq 2\sqrt{ \frac{2}{\sigma} ( Q(H, v, v) - Q(H, p_\phi(v), v) + \phi ) }, 
    \end{align*}
    where the third and the last inequalities follow from \eqref{lem:BT2.3-assump-g} the definition of $p_\phi(v)$. Hence, \eqref{equ:bound_delta_x_inexact} follows by taking square on both sides of the above inequality.
\end{proof}


%  Since we do not optimize subproblems accurately we need  to impose the following additional condition 
% \begin{align}
%     \label{equ:bound_delta_x_inexact}
%     \| p_\phi(x^k)-x^k \|_{H_k}^2 \leq 2 \theta (Q(H_k, x^{k}, x^k) - Q(H_k, x^{k+1}, x^k)), \ {\rm with\ some\ } \theta >1,  \forall k\ 
% \end{align}
% This condition is a relaxation of  \eqref{equ:bound_delta_x}, thus it holds automatically with $\theta=1$ in the case of exact subproblem minimization. 
% %Note that (\ref{equ:bound_delta_x_inexact}) will eventually be satisfied with the exact solution $\Delta x_k^*$ because, from Lemma \ref{lem:decrease_bound}, $\| \Delta x_k^* \|_{H_k}^2 \leq 2 (Q_k(0) - Q_k(\Delta x_k^*)) \leq 2 \theta (Q_k(0) - Q_k(\Delta x_k^*))$. 
% In the inexact case this assumption is easy to enforce and to check because all necessary quantities are available during the subproblem optimization (whether using coordinate descent or another approach). 
% This assumption is standard in optimization theory with inexact steps (for instance in trust-region methods) and is imposed to  achieve some minimal level of  the model improvement. 

 Applying \eqref{equ:bound_delta_x_inexact} instead of \eqref{equ:bound_delta_x} within the derivation of \eqref{eq:x2_bound_exact} and replacing $v$ and $p_\phi(v)$ in \eqref{equ:bound_delta_x_inexact} with $x^i, x^{i+1}$, respectively, 
we have 

\begin{align}\label{sub:bounding_the_function_values}
    &\sum_{i=0}^{k-1}M_{i+1}\|x^{i+1}-x^i\|^2  \\
    \nonumber&\leq M^\prime \sum_{i=0}^{k-1}\|x^{i+1}-x^i\|^2\leq \frac{ 8M^\prime}{\sigma} \sum_{i=0}^{k-1} (Q(H_i, x^{i}, x^i) - Q(H_i, x^{i+1}, x^i) + \phi_i)\\
    \nonumber& \leq \frac{8M^\prime }{\sigma \rho}  \sum_{i=0}^{k-1}( F(x^i) - F(x^{i+1})) + \frac{ 8M^\prime}{\sigma} \sum_{i=0}^{k-1} \phi_i = \frac{8M^\prime}{\sigma \rho} (F(x^0)-F(x^*)) + \frac{ 8M^\prime}{\sigma} \sum_{i=0}^{k-1} \phi_i.
\end{align}

% \begin{equation}
%     \sum_{i=0}^{k-1}\|x^{i+1}-x^i\|_{H_i-H_{i+1}}^2 \leq \frac{2\theta K_H }{\rho\sigma} (F(x^0)-F(x^*))
% \end{equation}
Hence bound   \eqref{eq:sumboundK} holds with appropriately chosen $K$ if the errors  $\phi_i$'s are summable. 

Before presenting the  main result, we state  the following auxiliary  lemma from \cite{Schmidtetal}. 
\begin{lemma}
    \label{lem:bach}
    Assume that the nonnegative sequence $\{u_k\}$ satisfies the following recursion for all $k \geq 1$
    \begin{align*}
...
Assume that for all  iterates $\{x^k\}$ of inexact Algorithm \ref{alg:ISTA-SD} \eqref{equ:bound_delta_x_inexact} holds for some $\phi_k>0$, then
\begin{align}
    \label{eq:bound_F_F*}
    F(x^k) - F(x^*) \leq \frac{1}{k} \left( \frac{\sigma (B_k + C)}{2} + (2A_k + \sqrt{C} + \sqrt{B_k}) A_k \right),
\end{align}
with
\begin{align*}
    A_k = \sum_{i=0}^{k-1} \sqrt{2M\phi_i}, ~~C = \frac{1}{\sigma}  \|x^0-x^*\|_{H_0}^2 + \frac{2(M_{\sigma}  +1 - \rho)}{\rho \sigma} (F(x^0)-F(x^*)), ~~B_k =  \frac{2(M_{\sigma}+1)}{\sigma} \sum_{i=0}^{k-1}\phi_i,
\end{align*}
where $x^*$ is an optimal solution of \eqref{prob:P}, $M_{\sigma}=4M^\prime/\sigma$ and $M$ and $\sigma$ are bounds of the smallest and largest eigenvalues of $H_k$.
\end{theorem}


\begin{proof}
    As is done in  \cite{Beck2009} and \cite{Schmidtetal},  we apply Lemma \ref{lem:subp_inexact}, sequentially, with $u=x^*$ and $p_{\phi}(v)=x^i$ for $i=1, \ldots, k+1$. Adding up resulting inequalities, we obtain  
    \begin{align}
        &\sum_{i=0}^{k-1} F(x^*) - \sum_{i=0}^{k-1} F(x^{i+1}) \\
        \nonumber &\geq \frac{1}{2} \sum_{i=0}^{k-1} \left( \|x^{i+1}-x^*\|_{H_{i}}^2 - \|x^i-x^*\|_{H_{i}}^2 \right)- \sum_{i=0}^{k-1} \epsilon_i- \sum_{i=0}^{k-1}\phi_i- \sum_{i=0}^{k-1} \langle \eta_i,x^* - x^{i+1} \rangle \\
        \label{equ:bound_F_diff}
        &= \frac{1}{2} \left( \|x^k-x^*\|_{H_k}^2-\|x^0-x^*\|_{H_0}^2 - \sum_{i=0}^{k-1}M_{i+1}\|x^{i+1}-x^i\|^2 \right) -\sum_{i=0}^{k-1}\epsilon_i - \sum_{i=0}^{k-1}\phi_i- \sum_{i=0}^{k-1}\langle \eta_i,x^* - x^{i+1} \rangle\\
        \nonumber &\geq \frac{1}{2} \left( - \|x^0-x^*\|_{H_0}^2 - \sum_{i=0}^{k-1}M_{i+1}\|x^{i+1}-x^i\|^2 \right) -\frac{(1-\rho)(F(x^0)-F(x^*))}{\rho} - \sum_{i=0}^{k-1}\phi_i- \sum_{i=0}^{k-1} \|\eta_i\| \cdot \|x^* - x^{i+1}\|.
    \end{align}

    Here we used the already established bound $\sum_{i=0}^{k-1}\epsilon_i \leq \frac{(1-\rho)(F(x^0)-F(x^*))}{\rho}$. We can now apply the bound \eqref{sub:bounding_the_function_values}
    with $M_{\sigma}=4M^\prime/\sigma$ and obtain
    \begin{align*}
        &\sum_{i=0}^{k-1} F(x^{i+1}) - \sum_{i=0}^{k-1} F(x^*) \\
        &\leq \frac{1}{2} \|x^0-x^*\|_{H_0}^2 + \frac{M_{\sigma}  + 1 - \rho}{\rho} (F(x^0)-F(x^*)) + (M_{\sigma} + 1) \sum_{i=0}^{k-1}\phi_i + \sum_{i=0}^{k-1} \|\eta_i\| \cdot \|x^* - x^{i+1}\|. 
    %   &\leq \frac{m B_k + C}{2} + (2A_k + \sqrt{\frac{C}{m}} + \sqrt{B_k}) A_k
    \end{align*}
    ...
    % \subsection{Bound $\| x^k - x^* \|$} % (fold)
    % \label{sub:bound2}

    Following  \cite{Schmidtetal} we use Lemma \ref{lem:bach} to bound $\|x^k-x^*\|^2 $. First, observe in the inequality \eqref{equ:bound_F_diff} that its left-hand side is always less than zero since $F(x^*) - F(x^{i+1}) \leq 0$ holds in every iteration. Setting the left-hand side to zero and moving $|| x^k - x^* ||^2_{H_k}$ to the left, we obtain 
    \begin{align*}
        &\|x^k-x^*\|_{H_k}^2  \\
        &\leq  \|x^0-x^*\|_{H_0}^2 + \sum_{i=0}^{k-1}M_{i+1}\|x^{i+1}-x^i\|^2  + 2\sum_{i=0}^{k-1}\epsilon_i + 2\sum_{i=0}^{k-1}\phi_i + 2\sum_{i=0}^{k-1} \sqrt{2M\phi_i} \cdot \|x^* - x^{i+1}\| \\
        &\leq \|x^0-x^*\|_{H_0}^2 
        + \frac{2M_{\sigma} }{\rho} (F(x^0)-F(x^*))  + 2M_{\sigma} \sum_{i=0}^{k-1} \phi_i
        + 2\sum_{i=0}^{k-1}\epsilon_i + 2\sum_{i=0}^{k-1}\phi_i + 2\sum_{i=0}^{k-1} \sqrt{2M\phi_i} \cdot \|x^* - x^{i+1}\|,
    \end{align*}
    hence,
    \begin{align*}
        &\|x^k-x^*\|^2  \leq \frac{1}{\sigma} \|x^k-x^*\|_{H_k}^2  \\
        &\leq \frac{1}{\sigma} \left( \|x^0-x^*\|_{H_0}^2 
        + \frac{2M_{\sigma} }{\rho} (F(x^0)-F(x^*)) + 2M_{\sigma} \sum_{i=0}^{k-1} \phi_i + 2\sum_{i=0}^{k-1}\epsilon_i + 2\sum_{i=0}^{k-1}\phi_i + 2\sum_{i=0}^{k-1} \sqrt{2M\phi_i} \cdot \|x^* - x^{i+1}\| \right).
    \end{align*}
    Now using Lemma \ref{lem:bach} with $S_k = \frac{1}{\sigma} \left ( \|x^0-x^*\|_{H_0}^2 + \frac{2M_{\sigma} }{\rho} (F(x^0)-F(x^*)) + 2M_{\sigma} \sum_{i=0}^{k-1} \phi_i + 2\sum_{i=0}^{k-1}(\epsilon_i + \phi_i) \right)$,  $\nu_i = 2\sqrt{2M\phi_i}$) and $\epsilon_i = \frac{1-\rho}{\rho} (F(x^i) - F(x^{i+1}))$ we obtain
    \begin{align*}
        & \|x^k-x^*\| \leq \sum_{i=0}^{k-1} \sqrt{2M\phi_i}  \\
        & 
        + \sqrt{\frac{1}{\sigma} ( \|x^0-x^*\|_{H_0}^2 + \frac{2(M_{\sigma}  +1 - \rho)}{\rho} (F(x^0)-F(x^*)) + 2M_{\sigma} \sum_{i=0}^{k-1} \phi_i + 2\sum_{i=0}^{k-1} \phi_i ) + (\sum_{i=0}^{k-1} \sqrt{2M\phi_i})^2}.
    \end{align*}
    Denoting $A_k = \sum_{i=0}^{k-1} \sqrt{2M\phi_i},  C = \frac{1}{\sigma}  \|x^0-x^*\|_{H_0}^2 + \frac{2(M_{\sigma}  +1 - \rho)}{\rho \sigma} (F(x^0)-F(x^*))$ and $B_k =  \frac{2(M_{\sigma}+1)}{\sigma} \sum_{i=0}^{k-1}\phi_i$, we obtain
    \begin{align*}
        \|x^k-x^*\| \leq A_k + \left( C + B_k + A_k^2 \right)^{1/2}.
    \end{align*}
    Since $A_i$ and $B_i$ are increasing sequences, we have, for $i < k$
    \begin{align*}
        \|x^i-x^*\| &\leq A_i + \left( \frac{1}{\sigma} ( \|x^0-x^*\|_{H_0}^2 + \frac{2(M_{\sigma}  +1 - \rho)}{\rho} (F(x^0)-F(x^*)) + B_i + A_i^2 \right)^{1/2} \\
        &\leq A_k + \left( \frac{1}{\sigma} ( \|x^0-x^*\|_{H_0}^2 + \frac{2(M_{\sigma}  +1 - \rho)}{\rho} (F(x^0)-F(x^*)) + B_k + A_k^2 \right)^{1/2} \\
        &\leq A_k + \sqrt{C} + \sqrt{B_k} + A_k.
    \end{align*}
    ...
    Using the bound on $\|x^i-x^*\|$ above we can now derive the main complexity bound using
        ...
        &\sum_{i=0}^{k-1} F(x^{i+1}) - \sum_{i=0}^{k-1} F(x^*) \\
        &\leq \frac{1}{2} \|x^0-x^*\|_{H_0}^2 + \frac{M_{\sigma}  + 1 - \rho}{\rho} (F(x^0)-F(x^*)) + (M_{\sigma} + 1) \sum_{i=0}^{k-1}\phi_i + \sum_{i=0}^{k-1} \|\eta_i\| \cdot \|x^* - x^{i+1}\| \\
        % &\leq \frac{1}{2} \|x^0-x^*\|_{H_0}^2 + \frac{M_{\sigma}  + 1 - \rho}{\rho} (F(x^0)-F(x^*)) + \sum_{i=0}^{k-1}\phi_i + \sum_{i=0}^{k-1} \|\eta_i\| \cdot \|x^* - x^{i+1}\|. \\
        &\leq \frac{\sigma (B_k + C)}{2} + (2A_k + \sqrt{C} + \sqrt{B_k}) A_k.
    \end{align*}
    Hence,
    \begin{align*}
        F(x^k) - F(x^*) &\leq \frac{1}{k}( \sum_{i=0}^{k-1} F(x^{i+1}) - \sum_{i=0}^{k-1} F(x^*) )\\
        &\leq \frac{1}{k} \left( \frac{\sigma (B_k + C)}{2} + (2A_k + \sqrt{C} + \sqrt{B_k}) A_k \right).
    \end{align*}
\end{proof}
...
\begin{align}
    ...
    E[\sum_{i=1}^{k} \sqrt{\phi_i}] &\leq \frac{ \sqrt{\bar{R} \bar\alpha}}{ 1 - \sqrt{\bar\alpha}} \\
    \label{equ:single_sum}
    E[(\sum_{i=1}^{k} \sqrt{\phi_i})^2] &\leq \frac{2\bar{R} \bar\alpha}{ (1 - \sqrt{\bar\alpha})^2} \\
    \label{equ:double_sum}
    E[\sqrt{\sum_{i=1}^{k}\phi_i}\sum_{i=1}^{k} \sqrt{\phi_i}] &\leq \frac{\bar{R} \sqrt{\bar\alpha} }{ \sqrt{1 - \bar\alpha} (1 - \sqrt{\bar\alpha})}.
\end{align} 
\end{lemma}
\begin{proof}
    First we note that 
    ...
    \end{align*}
    Similarly, we establish \eqref{equ:single_sum}. Observe:
    \begin{align*}
        E[(\sum_{i=1}^{k} \sqrt{\phi_i})^2]&=E[\sum_{i,j=1, i \neq j}^k\sqrt{\phi_i \phi_j} +\sum_{i=1}^k \phi_i] \\
        &= \sum_{i,j=1, i \neq j}^k E[\sqrt{\phi_i \phi_j}] +\sum_{i=1}^k E[\phi_i] \\
        &= \sum_{i,j=1, i \neq j}^k E[\sqrt{\phi_i}] E[\sqrt{\phi_j}] +\sum_{i=1}^k E[\phi_i]\\
        &\leq \sum_{i,j=1, i \neq j}^k \sqrt{E[\phi_i]} \sqrt{E[\phi_j]} +\sum_{i=1}^k E[\phi_i] \\
        &\leq \sum_{j=1}^k \sqrt{\bar{R}} \bar\alpha^{j/2} \sum_{i=1}^k \sqrt{\bar{R}} \bar\alpha^{i/2} +\sum_{i=1}^k \bar{R}\bar\alpha^i \\
        &\leq  (\sqrt{\bar{R} \bar\alpha} \frac{1 - \sqrt{\bar\alpha^k}}{1 - \sqrt{\bar\alpha}})^2 + \bar{R} \bar\alpha \frac{1 - \bar\alpha^k}{1 - \bar\alpha}\\
        &\leq  \frac{\bar{R} \bar\alpha}{(1 - \sqrt{\bar\alpha})^2} +  \frac{\bar{R} \bar\alpha}{1 - \bar\alpha} \leq \frac{2 \bar{R} \bar\alpha}{ (1 - \sqrt{\bar\alpha})^2}.
    \end{align*}
    
    To bound $E[\sqrt{\sum_{i=1}^{k}\phi_i}\sum_{i=1}^{k} \sqrt{\phi_i}]$, and hence establish \eqref{equ:double_sum}, we again use Jensen inequality - $E(C)^2\leq E(C^2)$ for any random variable $C$. Specifically,
    \begin{align}
        \label{equ:E_bound_double_sum1}
        (E[\sqrt{\sum_{i=1}^{k}\phi_i}\sum_{i=1}^{k} \sqrt{\phi_i}]) ^2 
        \leq E[ \sum_{l=1}^{k}\phi_l (\sum_{i=1}^{k} \sqrt{\phi_i})^2] 
        \leq  E[\sum_{l=1}^{k}\phi_l \sum_{i=1}^{k}\sum_{j=1}^{k} \sqrt{\phi_j} \sqrt{\phi_i}].
    \end{align}
    We now consider  three cases, and derive a  bound for each, respectively. 
    \begin{itemize}
    \item[(i)] For any fixed $l$, consider the case when $i \neq j \neq l$,
    \begin{align*}
        &E[\phi_l \sum_{i \neq j \neq l; i,j = 1}^k \sqrt{\phi_j} \sqrt{\phi_i}] \\
        &\leq E[\phi_l] \sum_{i \neq j \neq l; i,j = 1}^k \sqrt{E[\phi_j]} \sqrt{E[\phi_i]}] 
        \leq \bar\alpha^l \bar{R}^2 \sum_{i \neq j \neq l; i,j = 1}^k \bar\alpha^{i/2 + j/2} \\
        &\leq \bar\alpha^l \bar{R}^2 \sum_{i,j = 1}^k \bar\alpha^{i/2 + j/2} 
        \leq \bar\alpha^l \bar{R}^2 (\frac{\sqrt{\bar\alpha} (1 - \bar\alpha^{k/2})} {1 - \sqrt{\bar\alpha}})^2 
        \leq \bar\alpha^l \frac{\bar\alpha \bar{R}^2}{(1 - \sqrt{\bar\alpha})^2}.
    \end{align*}
    \item[(ii)] Next, we consider  $i \neq j = l$ or $j \neq i = l$, in which case we recall that $\phi$ is bounded and lies in an interval $[0, \Phi]$, hence $E[\phi_i^{3/2}]\leq \Phi^{1/2}E[\phi_i]$, and it follows that
    \begin{align*}
        &E[2 \sum_{i = 1}^{l-1} \phi_l^{3/2} \sqrt{\phi_i} + 2 \sum_{i=l+1}^k \phi_l^{3/2} \sqrt{\phi_i}] \\
        &= 2 E[\phi_l^{3/2}] E[ \sum_{i = 1}^{l-1}  \sqrt{\phi_i} + \sum_{i=l+1}^k \sqrt{\phi_i}] \\
        &\leq 2 \Phi^{1/2} E[\phi_l] (\sum_{i = 1}^{l-1}  \sqrt{E[\phi_i]} + \sum_{i=l+1}^k \sqrt{E[\phi_i]}) \\
        &\leq 2 \Phi^{1/2} \bar\alpha^l \bar{R}\sqrt{\bar{R}} (\sum_{i = 1}^{l-1}  \bar\alpha^{i/2} + \sum_{i=l+1}^k \bar\alpha^{i/2}) \\
        &\leq 2 \Phi^{1/2} \bar\alpha^l \bar{R}\sqrt{\bar{R}} \frac{\sqrt{\bar\alpha}(1-\bar\alpha^{k/2})}{1-\sqrt{\bar\alpha}}
        \leq  \bar\alpha^l \frac{2\bar{R} \sqrt{\bar\alpha \Phi \bar{R}}}{1-\sqrt{\bar\alpha}}.
    \end{align*}
    
    \item[(iii)] Finally, we consider the case when $i = j = l$. Again, we note $E[\phi_i^2]\leq \Phi E[\phi_i]$ from the fact that $\phi$ is bounded from above by $\Phi$. Then,
    % \begin{align*}
    %   E[\phi_l \sum_{i=1}^k \phi_i] 
    %   &= E[\phi_l \sum_{i=1}^{l-1} \phi_i + \phi_l^2 + \phi_l \sum_{i = l+1}^k \phi_i] \\
    %   &= E[\phi_l] \sum_{i=1}^{l-1} E[\phi_i] + E[\phi_l^2] + E[\phi_l] \sum_{i = l+1}^k E[\phi_i] \\
    %   &\leq \bar\alpha^l \bar{R} ( \bar{R}\sum_{i=1}^{l-1} \bar\alpha^i + \Phi + \bar{R}\sum_{i = l+1}^k \bar\alpha^i) \\
    %   &\leq \bar\alpha^l \bar{R} ( \bar{R}\frac{\bar\alpha(1-\bar\alpha^k)}{1-\bar\alpha} + \Phi)
    %   \leq \bar\alpha^l \bar{R}( \frac{\bar{R}\bar\alpha}{1-\bar\alpha} + \Phi)
    % \end{align*}
    \begin{align*}
           E[\phi_l^2]\leq \Phi E[\phi_l] \leq \Phi \bar{R} \bar\alpha^l.
    \end{align*}            
    \end{itemize} 
    
    Hence, accounting for  all three cases in one sum, we have
    \begin{align}
        \nonumber&E[\sum_{l=1}^{k}\phi_l \sum_{i=1}^{k}\sum_{j=1}^{k} \sqrt{\phi_j} \sqrt{\phi_i}] = \sum_{l=1}^k E[\phi_l \sum_{i=1}^{k}\sum_{j=1}^{k} \sqrt{\phi_j} \sqrt{\phi_i}] \\
        \nonumber&\leq \bar{R}\sum_{l=1}^k \bar\alpha^l \left( \Phi + \frac{2 \sqrt{\bar\alpha \Phi \bar{R}}}{1-\sqrt{\bar\alpha}} + \frac{\bar\alpha \bar{R}}{(1 - \sqrt{\bar\alpha})^2}  \right) \\
        \label{equ:E_bound_double_sum2}
        &\leq  \frac{\bar{R}\bar\alpha}{1-\bar\alpha} \left( \Phi + \frac{2 \sqrt{\bar\alpha \Phi \bar{R}}}{1-\sqrt{\bar\alpha}} + \frac{\bar\alpha \bar{R}}{(1 - \sqrt{\bar\alpha})^2}  \right).
    \end{align}
    From (\ref{equ:E_bound_double_sum1}) and (\ref{equ:E_bound_double_sum2}) it follows that
    \begin{align*}
        E[\sqrt{\sum_{i=1}^{k}\phi_i}\sum_{i=1}^{k} \sqrt{\phi_i}]
        &\leq \sqrt{ E[\sum_{l=1}^{k}\phi_l \sum_{i=1}^{k}\sum_{j=1}^{k} \sqrt{\phi_j} \sqrt{\phi_i}]} \\
        &\leq \sqrt{ \frac{\bar{R}\bar\alpha}{1-\bar\alpha} \left( \Phi + \frac{2 \sqrt{\bar\alpha \Phi \bar{R}}}{1-\sqrt{\bar\alpha}} + \frac{\bar\alpha \bar{R}}{(1 - \sqrt{\bar\alpha})^2}  \right)  } \nonumber \\
        &\mbox{since $\Phi \leq \bar{R}$} \nonumber \\
        &\leq \sqrt{ \frac{\bar{R}^2\bar\alpha}{1-\bar\alpha} \left( 1 + \frac{2 \sqrt{\bar\alpha}}{1-\sqrt{\bar\alpha}} + \frac{\bar\alpha}{(1 - \sqrt{\bar\alpha})^2}  \right)}. \nonumber \\
        & = \frac{\bar{R} \sqrt{\bar\alpha} }{ \sqrt{1 - \bar\alpha} (1 - \sqrt{\bar\alpha})} \nonumber.
    \end{align*}
