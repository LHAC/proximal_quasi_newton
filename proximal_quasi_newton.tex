...
\begin{align}
    \label{eq:bound_F_F*}
    F(x^k) - F(x^*) \leq \frac{1}{k} \left( C + B_k + A_k \right),
\end{align}
with
\begin{align*}
    A_k = \delta\sum_{i=0}^{k-1} \sqrt{2M\phi_i}, ~~
    C = \frac{1}{2}  \|x^0-x^*\|_{H_0}^2 + \frac{(1-\rho)(F(x^0)-F(x^*))}{\rho}, ~~
    B_k = \sum_{i=0}^{k-1}\phi_i,
\end{align*}
where $x^*$ is an optimal solution of \eqref{prob:P}, $\|x - x^*\| \leq \delta $ for all $x \in \Rmbb^n$  and $M$ is the bound of the largest eigenvalues of $H_k$.
\end{theorem}
...
\begin{proof}
    ...
    \begin{align}
        ...
        + \sum_{i=0}^{k-1}\phi_i 
        + \delta\sum_{i=0}^{k-1} \sqrt{2M\phi_i} = C + B_k + A_k. \\
        \nonumber &\mbox{where the last equality follows from the definitions of $C, B_k$ and $A_k$}.
    \end{align}

    Hence,
    \begin{align*}
        F(x^k) - F(x^*) &\leq \frac{1}{k}( \sum_{i=0}^{k-1} F(x^{i+1}) - \sum_{i=0}^{k-1} F(x^*) )\\        
        &\leq \frac{1}{k} \left( C + B_k + A_k \right).
    \end{align*}
\end{proof}
