...
Here are the assumptions made in our analysis.
...
There exists a positive constant $\delta$ such that for all  iterates $\{x^k\}$ of inexact Algorithm \ref{alg:ISTA-SD}:
\begin{align*}
     \|x^k - x^*\| \leq \delta
\end{align*} 
\assume 
\label{assub:bound_h}
There exists a positive constant $M$ such that for all  iterates $\{x^k\}$ of inexact Algorithm \ref{alg:ISTA-SD}: 
\begin{align*}
     0 \prec H_k \prec MI
\end{align*} 
\end{assumption}
We note that while it is possible to remove Assumption~\ref{assub:bound_level_set} by directly establishing a uniform bound on $\|x^k - x^*\|$ (rf. \cite{OML,Schmidtetal}), here we instead make it an explicit assumption, mainly for simplicity of the presentation. This allows us to provide a much shorter and more concise proof than that given in \cite{OML}. 
