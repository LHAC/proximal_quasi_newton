...
Let $x^{k+1} \in \dom(F)$ be any point such that $Q(H_k, x^{k+1},x^k)-\min_{y\in\br^n} Q(H_k, y,x^k)\leq \phi_k$ holds with any positive number $\phi_k > 0$. Then the decrease on the function $Q$ can be bounded from below by a quadratic function in terms of the step size, i.e., $\|x^{k+1} - x^k\|$,
\begin{align}
...
\begin{lemma}
	...
	Consider $k$th iteration of the inexact Algorithm \ref{alg:ISTA-SD} with errors $\phi_k>0$ (cf. \eqref{lem:BT2.3-assump-g}). Let $\Delta F_k := F(x^k) - F(x^*)$. Then for some fixed large positive constant $\theta > 0$ either one of the two cases, i.e., \eqref{equ:bound_F_inexact_case1} and \eqref{equ:bound_F_inexact_case2}, must hold,
	\begin{align}
	...
	\end{align}
	where $t_k, b_k$ and $c_k$ are given below,
	\begin{align*}
	    t_k=\frac{\sigma_k}{2}(\frac{\theta-1}{\theta M_k})^2 - \frac{1+\theta}{\theta^2\sigma_k}- \frac{1}{2\theta^2M_k}, ~b_k = (\theta D_{\Xcal_0}+  \frac{2(1+\theta)L_g}{\sigma_k})\sqrt{2M_k}, ~c_k=\frac{\rho t_k\sigma_k^2\theta^2}{(D_{\Xcal_0}\theta\sigma_k + 2L_g(1+\theta))^2}.
	\end{align*}
\end{lemma}
\begin{proof}	                
	...
	Apply \eqref{equ:bound_F_u_w} with $u = x^k, w = x^*$ and $v = x^{k+1}$ and we obtain,
	\begin{align}
	\label{equ:_bound_F_xk_xk1}
	    \Delta F_k = F(x^k) - F(x^*)
	    \leq \|\nabla f(x^k)+\gamma_g^{k+1}\|\|x^k-x^*\| + 2L_g \|x^k-x^{k+1}\|. 
	\end{align}
	...
	\[
		\Delta F_k\leq D_{\Xcal_0} \|\nabla f(x^k)+\gamma_{g, \phi}^{k+1}\| +  \frac{2(1+\theta)L_g\sqrt{2M_k\phi_k}}{\sigma_k}
		\leq (\theta D_{\Xcal_0}+  \frac{2(1+\theta)L_g}{\sigma_k})\sqrt{2M_k\phi_k}.
	\]
	...
	\begin{align*}
		F(x^{k+1})-F(x^k)=\Delta F_{k+1}-\Delta F_k\geq c_k\Delta_k^2,\quad
	    \mbox{with, }c_k=\frac{\rho t_k\sigma_k^2\theta^2}{(D_{\Xcal_0}\theta\sigma_k + 2L_g(1+\theta))^2}.
	\end{align*}
