...
\end{align}
\begin{proof}
     From the first-order optimality conditions, the following equality holds with $\gamma_g^{k+1}$ being one of the subgradient of $g(\cdot)$ at $x^{k+1}$: 
    \begin{align*}
        % \label{equ:bound_step_size_1st_optima}
...
\end{lemma}
 ...
 We will denote $F(x^k)-F^*$ by $\Delta_k$.  Our goal is to bound $\Delta_k$ from above  in terms of $1/k$ which we will achieve by deriving a lower bound  on $\frac{1}{\Delta_k}$ in terms of $k$. 
 ...
 \Delta_k\leq (D+\frac{2L_g}{\sigma_k}) \|\nabla f(x^k)+\gamma_g^{k+1}\|.
 \end{equation}
 ...
 \begin{align*}
     ...
     + 2L_g \|x^k-x^{k+1}\|. 
 \end{align*}
  Substituting the bounds on  $\| x^{k+1} - x^k\|$ and   $\| x^{k} - x^*\|$
 in the above bound on $\Delta_k$ we get
...
Finally, combining the lower bound on $F(x^{k+1})-F(x^k)$ together with the upper bound on $\Delta_k^2$  we can conclude that
\[
F(x^{k+1})-F(x^k)=\Delta_{k+1}-\Delta_k\geq  \frac{\rho\sigma_k}{2M_k^2 (D+\frac{2L_g}{\sigma_k})^2}\Delta_k^2,
\]
which establishes \eqref{eq:mainbnd} with $c_k= \frac{\rho\sigma_k^3}{2M_k^2 (D\sigma^k+2L_g)^2}$. 

Dividing by $\Delta_{k+1}\Delta_k\leq \Delta_{k}^2$ we have
\[
\frac{1}{\Delta_{k}}-\frac{1}{\Delta_{k+1}}\geq c_k.
\]

Let us note that for all $k$ $c_k\geq  \frac{\rho\sigma^3}{2M^2 (DM+2L_g)^2}$. 
Summing the above expression for  $i=0, \ldots, k-1$ we have
\[
\frac{1}{\Delta_{k}}\geq \sum_{i=0}^{k-1}c_i+\frac{1}{\Delta_{0}} \geq \sum_{i=0}^{k-1}c_i
\]
which finally implies
\[
\Delta_k=F(x^k)-F^*\leq \frac{1}{\sum_{i=0}^{k=1} c_i} \leq  \frac{2M^2 (DM+2L_g)^2}{\rho\sigma^3}\frac{1}{k}
\]

Let us note that if $H_k=LI$ for all $k$, as in standard proximal gradient method, where $L$ is the Lipschitz constant of $\nabla f(x)$, then
the bound becomes
\[
F(x^k)-F^*\leq   \frac{ 2(DL+2L_g)^2}{\rho L}\frac{1}{k}\approx \frac{ 2D^2L}{k},
\]
if $L_g\ll DL$. This is a bound similar to $\frac{2\|x^0-x^*\|^2L}{k}$ established for proximal gradient method, assuming that $D$ is comparable to
$\|x^0-x^*\|$. 
